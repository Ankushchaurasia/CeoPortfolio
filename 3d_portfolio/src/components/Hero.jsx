...
const Hero = () => {
 ...
 <section id="home" className="min-h-screen flex items-center pt-20 relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 flex flex-col-reverse lg:flex-row items-center gap-12 lg:gap-20">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={staggerContainer} className="flex-1 text-center lg:text-left">
          ...
          <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row justify-center lg:justify-start gap-4">
            <ScrollLink to="contact" smooth={true} offset={-70} className="bg-gradient-to-r-theme text-white font-bold py-4 px-8 rounded-full hover:opacity-90 transition-opacity cursor-pointer flex items-center justify-center gap-2">
              Let's Connect <ChevronRight size={20} />
            </ScrollLink>
            <ScrollLink to="services" smooth={true} offset={-70} className="bg-surface text-white font-bold py-4 px-8 rounded-full hover:bg-surface-hover transition-colors cursor-pointer border border-white/10">
              Explore Expertise
            </ScrollLink>
          </motion.div>
        </motion.div>
